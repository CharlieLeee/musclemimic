...
def _load_c3d(path: str) -> tuple[np.ndarray, list[str], float]:
    ...
    # points shape: (4, N_markers, N_frames) -> transpose to (N_frames, N_markers, 3)
    pts = c["data"]["points"][:3, :, :]  # drop residual row
    positions = np.transpose(pts, (2, 1, 0)).astype(np.float32)

    # Unit conversion: ensure meters
    units = c["parameters"]["POINT"]["UNITS"]["value"]
    if units and units[0].lower() == "mm":
        positions /= 1000.0

    return positions, labels, frame_rate
...
def _build_bone_indices(
    ...
    bat_markers = sorted(
        [l for l in labels if l.startswith("Marker") and l[6:].isdigit()],
...
def main():
    ...
    with tabs.add_tab("Controls", icon=viser.Icon.SETTINGS):
        ...
        with server.gui.add_folder("Playback"):
            ...
            def _(_ev) -> None:
                ...
                pause_button.icon = (
                    viser.Icon.PLAYER_PAUSE if not paused[0] else viser.Icon.PLAYER_PLAY
                )
        ...
        with server.gui.add_folder("Info"):
            info_html = server.gui.add_html(
                f"<b>File:</b> {args.c3d_file.split('/')[-1]}<br>"
    ...
    def _update_status():
        ...
        t = f / frame_rate
        status_html.content = (
            f"<b>{state}</b> &nbsp; Frame {f}/{n_frames - 1} &nbsp; "
            f"t={t:.3f}s &nbsp; Speed: {speed[0]:.2f}x"
        )
    ...
    def _build_bone_geometry(
        ...
        pts = np.empty((len(pairs), 2, 3), dtype=np.float32)
        for i, (a, b) in enumerate(pairs):
            pts[i, 0] = frame_pos[a]
            pts[i, 1] = frame_pos[b]
        cols = np.stack([colors, colors], axis=1)
        return pts, cols
    ...
    def _update_frame(frame_idx: int):
        ...
        with server.atomic():
            ...
            if trail_len > 0 and frame_idx > 0:
                ...
                # Flatten all markers across trail frames
                t_len = trail_positions.shape[0]
                trail_pts = trail_positions.reshape(-1, 3)
                # Fade alpha: older = more transparent
                alphas = np.linspace(0.1, 0.8, t_len)
                trail_cols = np.zeros((t_len, n_markers, 3), dtype=np.uint8)
                for t in range(t_len):
                    trail_cols[t] = (marker_colors * alphas[t]).astype(np.uint8)
                trail_cols = trail_cols.reshape(-1, 3)
